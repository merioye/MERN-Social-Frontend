        ...
        if (user) {
            if (!socketRef.current) {
                ...
                socketRef.current.on(
                    "receive-message",
                    (message, room, recordedTime, notification) => {
                ...
                socketRef.current.on("typing", (room, name) => {
                    ...
                    if (
                        timerId.current &&
                        Object.keys(prevIds).includes(room)
                    ) {
                    ...
                    (room, updatedChat, newMessage) => {
                        ...
                        setSelectedConversationInfo((prevInfo) => {
                            if (id === room) {
                                return {
                                    ...prevInfo,
                                    grpSubject: updatedChat.grpSubject,
                                };
                            } else {
                                return prevInfo;
                            }
                    ...
                    (room, updatedChat, newMessage) => {
                        ...
                        setSelectedConversationInfo((prevInfo) => {
                            if (id === room) {
                                return {
                                    ...prevInfo,
                                    grpEditInfo: updatedChat.grpEditInfo,
                                };
                            } else {
                                return prevInfo;
                            }
                ...
                socketRef.current.on(
                    "addNewParticipantsToGrp",
                    (room, updatedChat, newMessages) => {
                        let id;
                        setSelectedConversationId((prevId) => {
                            id = prevId;
                            return prevId;
                        ...
                        setPosts((posts) => {
                            let updatedPosts = posts.map((post) => {
                                if (String(post._id) === String(postId)) {
                                    let updatedPost = post;
                                    updatedPost.postComments.unshift(
                                        newComment
                                    );
                                    return updatedPost;
                                } else {
                ...
                socketRef.current.on(
                    ...
                    (postId, commentId, newLike, isLiked) => {
                        setPosts((posts) => {
                            let updatedPosts = posts.map((post) => {
                                if (String(post._id) === String(postId)) {
                                    let updatedPost = post;
                                    let updatedPostComments =
                                        updatedPost.postComments.map(
                                            (postComment) => {
                                                if (
                                                    String(postComment._id) ===
                                                    String(commentId)
                                                ) {
                                                    if (isLiked) {
                                                        let updatedComment =
                                                            postComment;
                                                        updatedComment.commentLikes.push(
                                                            newLike
                                                        );
                                                        ...
                                                        let updatedComment =
                                                            postComment;
                                                        let updatedLikes =
                                                            updatedComment.commentLikes.filter(
                                                                (
                                                                    commentLike
                                                                ) => {
                                                                    return (
