async function checkUsernameAvailable(
    ...
    try {
        ...
        const res = await fetch(
            `${process.env.REACT_APP_API_BASE_URL}/username`,
            {
                method: "POST",
                credentials: "include",
                headers: {
        ...
        if (!isValid) {
            usernameSpanRef.current.textContent =
        ...
        event.target.previousElementSibling.lastElementChild.style.color =
            "red";
        console.log(e);
    }
}
