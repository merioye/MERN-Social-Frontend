...
const ConversationHeader = ({ setShowUpdateGrp, setShowConversation }) => {
    ...
    const { _id, users, isGrp, grpAvatar, grpSubject } =
        selectedConversationInfo;
    ...
    return (
        <div className="conversation-header">
            <div className="back-and-chat-name">
                <motion.i
                    className="fas fa-arrow-left chat-back-btn"
                    onClick={handleConversationMessagesBackBtn}
                                ...
                                {isGrp ? (
                                    users.map((user, ind) => {
                                        ...
                                        if (ind === users.length - 1) {
                                            return `${n[0].toUpperCase()}${n.slice(
                                                1
                                            )}`;
                                        } else {
                                            return `${n[0].toUpperCase()}${n.slice(
            ...
            {isGrp && (
                <motion.i
                    className="fas fa-eye view-grp-details-btn"
                    onClick={() => setShowUpdateGrp(true)}
                    initial={{ background: "#fff" }}
                    whileTap={{ background: "rgba(53, 53, 53, 0.05)" }}
                ></motion.i>
            )}
