...
const ConversationMessages = () => {
    ...
    useEffect(() => {
        const notification = messagesNotifications
            .reverse()
            .find((msgNotification) => {
                return (
                    msgNotification.notificationChatId ===
                    selectedConversationId
                );
            });
        ...
        const updateClickedMessagesNotification = async () => {
            try {
                const res = await fetch(
                    ...
                    }/notifications/${selectedConversationId}?&type=${"message"}`,
                    {
                        method: "PUT",
                        credentials: "include",
                    }
                ...
                setMessagesNotifications((prevMessagesNotifications) => {
                    let arr = prevMessagesNotifications.filter((notif) => {
                        return (
                            notif.notificationChatId !== selectedConversationId
                        );
                    });
                    return arr;
                });
        ...
        if (
            notification &&
            notification.messageNotificationReceivers.includes(user._id)
        ) {
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            ...
            try {
                const res = await fetch(
                    `${process.env.REACT_APP_API_BASE_URL}/messages/${selectedConversationId}`,
                    {
                        credentials: "include",
                    }
                );
    ...
    useEffect(() => {
        conversationMessagesRef.current.scrollTo({
            top: conversationMessagesRef.current.scrollHeight,
            left: 0,
        });
            ...
            <div className="conversation-messages-wrapper">
                ...
                >
                    {messages.length ? (
                        [
                            ...new Map(
                                messages.map((item) => [item["_id"], item])
                            ).values(),
                        ].map((message) => {
                            return (
                                <ConversationMessage
                ...
                {Boolean(selectedConversationInfo.isGrp) ? (
                    selectedConversationInfo.grpSendMessages === "all" ? (
                        ...
                        />
                    ) : selectedConversationInfo.grpAdmins.includes(
                          user._id
                      ) ? (
                        <SendMessage
                            selectedFile={selectedFile}
                        ...
                        <div className="only-admins-can-send-messages">
