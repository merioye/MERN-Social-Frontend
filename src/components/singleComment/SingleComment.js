...
const SingleComment = ({
    ...
    const [isCommentOptionsBtnVisible, setIsCommentOptionsBtnVisible] =
        useState(false);
    ...
    useEffect(() => {
        if (
            user.role === "admin" ||
            commentAuthor.username === user.username ||
    ...
    const handleCommentLike = async () => {
        ...
        try {
            const res = await fetch(
                ...
                }/posts/${postId}/comments/${_id}?liked=${!liked}`,
                {
                    credentials: "include",
                }
            );
            ...
            <div className="post-single-comment">
                <Link to={`/profile/${commentAuthor._id}`}>
                    <img
                        ...
                        alt="profile"
                        className="single-comment-profile-image"
                    />
                </Link>
                ...
                <div className="single-comment-text">
                    ...
                    >
                        <h4>{formatName(commentAuthor.name)}</h4>
                    </Link>
                    <p> {commentText} </p>
                    {user.role === "admin" ||
                    commentAuthor.username === user.username ||
                    postAuthor.username === user.username ? (
                        <span
                            className="single-comment-options"
                            onClick={() =>
                    ...
                    <div
                        className="single-comment-options-list"
                        style={
                            showCommentOptions
                                ? { display: "block" }
                                : { display: "none" }
                    ...
                    >
                        ...
                        {postAuthor.username !== user.username ||
                        commentAuthor.username === postAuthor.username ? (
                            <div
                ...
                {commentImage ? (
                    <div className="single-comment-image-wrapper">
                        <img
                            src={commentImage}
                            alt="commentImage"
                            className="single-comment-image"
                        />
                    </div>
                ) : null}
