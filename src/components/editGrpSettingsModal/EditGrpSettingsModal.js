        ...
        try {
            if (fromInfo) {
                ...
                const res = await fetch(
                    `${process.env.REACT_APP_API_BASE_URL}/chats/${selectedConversationInfo._id}?type=grpEditInfo`,
                    {
                        method: "PUT",
                        credentials: "include",
                        body: formData,
                ...
                let updatedChats = chats.filter((cht) => {
                    if (cht._id === data.updatedChat._id) {
                        const { grpEditInfo, lastMessage, lastMessageDate } =
                            data.updatedChat;
                        modifiedChat = {
                            ...cht,
                ...
                setSelectedConversationInfo({
                    ...selectedConversationInfo,
                    grpEditInfo: data.updatedChat.grpEditInfo,
                });
                ...
                const res = await fetch(
                    `${process.env.REACT_APP_API_BASE_URL}/chats/${selectedConversationInfo._id}?type=grpSendMessages`,
                    {
                        method: "PUT",
                        credentials: "include",
                        body: formData,
                ...
                setSelectedConversationInfo({
                    ...selectedConversationInfo,
                    grpSendMessages: data.updatedChat.grpSendMessages,
                });
                setMessages([...messages, newMessage]);
                setShowLoader(false);
                setShowModal(false);
    ...
    return (
        <>
            <div className="edit-grp-settings-modal-container">
                <motion.div
                    className="edit-grp-settings-modal"
                    ref={modal}
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                ...
                >
                    ...
                    <form>
                        <div style={{ marginBottom: "10px" }}>
                            ...
                            />
                            <label htmlFor="all-participants">
                                All participants
                            </label>
                        </div>
                        <div>
