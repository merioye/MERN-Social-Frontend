...
const SingleNotification = ({
    notification,
    setViewNotificationDetails,
    setClickedNotification,
}) => {
    const [showDeleteNotificationOption, setShowDeleteNotificationOption] =
        useState(false);
    ...
    const [showDeleteNotificationLoader, setShowDeleteNotificationLoader] =
        useState(false);

    const { onlineUsers, setNotifications, setMoreNotificationsToSkip } =
        useContext(Context);
        ...
        if (notification.notificationType === "follow") {
            ...
            const updateClickedNotification = async () => {
                try {
                    ...
                    navigate(
                        `/profile/${notification.notificationUserProfileId}`
                    );
                } catch (e) {
    ...
    const handleNotificationDeleteClick = async (e) => {
        ...
        try {
            const res = await fetch(
                `${process.env.REACT_APP_API_BASE_URL}/notifications/${notification._id}`,
                {
                    method: "DELETE",
                    credentials: "include",
                }
        ...
        <>
            ...
            >
                ...
                {notification.personId &&
                    onlineUsers.includes(String(notification.personId)) && (
                        <i
                ...
                <motion.span
                    ...
                    onClick={handleNotificationOptionClick}
                    style={
                        showDeleteNotificationOption
                            ? { background: "#E4E6EB", color: "blue" }
                            : { background: "#E4E6EB", color: "black" }
                    }
                    id="deleteNotificationOption"
                    initial={{ scale: 1 }}
                    whileTap={{ scale: 0.85 }}
                >
                    ...
                ...
                <div
                    className="notification-options-list"
                    style={
                        showDeleteNotificationOption
                            ? { display: "block" }
                            : null
                    }
                    ref={deleteNotificationOption}
                >
